...
func start_pressed_animation() -> void:
	...
	var duration := 0.20
	tween.tween_property(start_button,"theme_override_colors/font_color",pressed_color,duration).set_trans(Tween.TRANS_BOUNCE)
	tween.tween_property(start_button,"theme_override_colors/font_color",hover_color,duration).set_trans(Tween.TRANS_BOUNCE)
	tween.set_loops()
